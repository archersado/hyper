...
import { EngineManager, RenderObject, Render } from '@hset/hyper-core';
...
let engine: Render;
export default function getHostConfig(hyperManager: EngineManager) {
    const config = {
        now: Date.now,
        ...
        getRootHostContext(rootContainerInstance: any) {
            if (!engine) {            
                ...
                engine = hyperManager.use(engineType, parsedConfig);
                ...
                if (object) {
                    for (const key in updateQueue) {
                        if (isEventProps(key)) {
                            ...
                            object.on(eventName, (options: any) => eventBind.call(instance, options, object));
                        }
